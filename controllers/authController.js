...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changePasswordAfter(decoded.iat)) return next(new AppError('This user recently changed his password . Please log in again', 401))
...
exports.fakeProtect = catchAsync(async (req, res, next) => {
  let token
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) token = req.headers.authorization.split(' ')[1]
  if (!token) return next(new AppError('You  do not have access to request data from this server !', 401))
